...
class VideoViewLayouter {
    ...
    func layoutSessions(_ sessions: [VideoSession], fullSession: VideoSession?, inContainer container: NSView) {
    ...
    func reponseSessionOfEvent(_ event: NSEvent, inSessions sessions: [VideoSession], inContainerView container: NSView) -> VideoSession? {
...
private extension VideoViewLayouter {
    func viewListFromSessions(_ sessions: [VideoSession], maxCount: Int, ignorSession: VideoSession?) -> [NSView] {
        var views = [NSView]()
        for session in sessions {
